...
def apply_smote_to_sequences(input_ids, attention_masks, labels, random_state=42):
    try:
        ...
        # **添加警告說明**
        print("⚠️  警告：SMOTE 對文本數據的重新採樣會產生合成的 token 序列")
        print("⚠️  合成樣本可能包含不存在的詞彙組合，僅用於平衡訓練數據")
        
        # 將多標籤轉換為組合標籤進行 SMOTE
        label_combinations = []
        for label_row in labels:
            combo_str = ''.join(map(str, label_row))
            label_combinations.append(int(combo_str, 2))
        
        unique_combos, combo_counts = np.unique(label_combinations, return_counts=True)
        min_samples = min(combo_counts)
        max_samples = max(combo_counts)
        ...
        # **計算目標樣本數（使用最多類別的樣本數）**
        target_samples_per_class = max_samples
        total_target_samples = len(unique_combos) * target_samples_per_class
        
        print(f"SMOTE 目標：每個類別 {target_samples_per_class} 樣本")
        print(f"預期總樣本數：{total_target_samples}")
        
        # **問題核心：SMOTE 在 token 級別進行插值**
        # 準備 SMOTE 輸入：將 input_ids 和 attention_masks 合併
        features = np.concatenate([input_ids, attention_masks], axis=1)
        
        smote = SMOTE(random_state=random_state, k_neighbors=k_neighbors)
        # **這裡 SMOTE 會在數值特徵空間中創建合成樣本**
        # 對於文本來說，這意味著在 token ID 之間進行線性插值
        # 例如：token_id_1 = 156, token_id_2 = 234 → 合成樣本可能得到 195.3
        ...
        # **問題：將浮點數強制轉換為整數 token ID**
        # 這會產生不存在於詞彙表中的 token ID
        # **添加合成樣本品質檢查**
        # 檢查生成的 token ID 是否在合理範圍內
        vocab_size = 21128  # BERT/RoBERTa 中文詞彙表大小
        invalid_tokens = np.sum((resampled_input_ids < 0) | (resampled_input_ids >= vocab_size))
        if invalid_tokens > 0:
            print(f"⚠️  警告：生成了 {invalid_tokens} 個無效的 token ID")
            # 將無效 token 替換為 [UNK] token (ID: 100)
            resampled_input_ids = np.clip(resampled_input_ids, 0, vocab_size - 1)
        
        print(f"原始樣本數: {len(labels)} -> 重新採樣後樣本數: {len(resampled_labels)}")
        print("✅ SMOTE 重新採樣完成（包含合成文本樣本）")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(all_input_ids)):
    ...
    print(f"{'='*40}")
    print(f"原始完整數據集: {len(original_labels)} 樣本")
    ...
    analyze_data_distribution(train_labels, f"Fold {fold+1} 訓練數據分佈 (SMOTE前)")
    
    # 應用 SMOTE 重新採樣
    train_inputs, train_masks, train_labels_resampled = apply_smote_to_sequences(
        train_inputs, train_masks, train_labels, random_state=42
    )
    
    analyze_data_distribution(train_labels, f"Fold {fold+1} SMOTE 重新採樣後數據分佈")
    print(f"SMOTE前訓練樣本數: {len(train_labels)} -> SMOTE後: {len(train_labels_resampled)}")
    ...
    train_dataset = MultiLabelDataset(train_inputs, train_masks, train_labels_resampled)
...
# 保存最佳模型
if best_trainer is not None:
    best_trainer.save_model("./roberta_3label_finetuned_model")  # 修正：改為 roberta
    tokenizer.save_pretrained("./roberta_3label_finetuned_model")  # 修正：改為 roberta
...
plt.title('Memory vs Performance (RoBERTa)')  # 修正：改為 RoBERTa
...
plt.ylabel('Macro F1 Score')
plt.title('Inference Time vs Performance (RoBERTa)')  # 修正：改為 RoBERTa
...
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.title('Performance Across Folds (RoBERTa)')
plt.legend()

plt.tight_layout()
plt.savefig('./roberta_comprehensive_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
...
# 14. 交叉驗證結果匯總（修正：將 Average Attention Weight 改為6位小數）
...
# 保存最佳模型
if best_trainer is not None:
    best_trainer.save_model("./roberta_3label_finetuned_model")
    tokenizer.save_pretrained("./roberta_3label_finetuned_model")
    print(f"\n最佳模型已保存! (Macro F1: {best_f1:.3f})")
else:
    print("\n警告: 沒有找到可保存的模型")
