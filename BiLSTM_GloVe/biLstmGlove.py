...
def load_glove_embeddings(vocab, embedding_dim=100):
    """模擬 GloVe 詞向量（實際應用中可載入預訓練的中文詞向量）"""
...
class BiLSTMClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_labels, embeddings=None, dropout=0.3):
        ...
        self.fc = nn.Linear(hidden_dim * 2, num_labels)  # *2 因為是雙向
    ...
    def forward(self, x, return_attention=False):
        ...
        output = self.fc(attended)  # (batch_size, num_labels)
...
# 7. 訓練函數
def train_model(model, train_loader, val_loader, num_epochs=10, lr=0.001, pos_weights=None):
...
for fold, (train_idx, val_idx) in enumerate(kf.split(sequences)):
    ...
    print("開始訓練...")
    train_model(model, train_loader, val_loader, num_epochs=10, lr=0.001, pos_weights=pos_weights)
    print("進行模型評估...")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.eval()
    all_predictions = []
...
plt.title('Memory vs Performance (BiLSTM+GloVe)')
...
plt.ylabel('Macro F1 Score')
plt.title('Inference Time vs Performance (BiLSTM+GloVe)')
...
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.title('Performance Across Folds (BiLSTM+GloVe)')
plt.legend()

plt.tight_layout()
plt.savefig('./bilstm_glove_comprehensive_analysis.png', dpi=300, bbox_inches='tight')
plt.close()
...
print("\nBiLSTM+GloVe 模型特點:")
print("- 使用雙向 LSTM 捕捉文本的上下文信息")
print("- 集成注意力機制提高模型可解釋性")
print("- 使用 GloVe 詞向量進行文本表示")
