...
def plot_confusion_matrix_panel(
    ...
    output_path: Optional[Path] = None,
    filename_stem: str = "confusion_matrices",
    title: str = "Confusion Matrices"
) -> Optional[plt.Figure]:
    """Create panel of confusion matrices for multiple models.
    
    Args:
        cm_dict: Dict mapping model names to confusion matrices
        output_path: Directory to save figure (None for no save)
        filename_stem: Filename without extension
        title: Figure title
    ...
    for idx, model in enumerate(models):
        ...
        axes[idx].set_title(model, fontsize=12)
...
def plot_class_distribution(
    ...
    output_path: Optional[Path] = None,
    filename_stem: str = "class_distribution",
    class_names: List[str] = None
) -> Optional[plt.Figure]:
    """Create side-by-side class distribution comparison.
    
    Args:
        y_data: Dict mapping data split names to label arrays
        output_path: Directory to save figure (None for no save)
        filename_stem: Filename without extension
        class_names: Names for classes (default: ['Class 0', 'Class 1'])
    ...
    for idx, (split_name, labels) in enumerate(y_data.items()):
        counts = pd.Series(labels).value_counts().sort_index()
        colors = ["#e74c3c", "#3498db"]
        axes[idx].bar(class_names, counts.values, color=colors, alpha=0.7, edgecolor='black')
        axes[idx].set_title(f"{split_name} Distribution", fontsize=12)
        axes[idx].set_ylabel("Sample Count", fontsize=11)
        axes[idx].grid(True, alpha=0.3, axis='y')
        
        # Add value labels on bars
        for bar, count in zip(axes[idx].patches, counts.values):
